...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const MyHomePage(title: 'GL Terminal 1.20'),
      routes: {
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  List<String> _flowControls = <String>[
    'none',
    'hardware',
  ];
  ...
  Timer? _readTimer = null;
  bool _readTimerCancel = false;

  List<String> refreshPrtNames() {
    //var portNames = await BasicDev.getPortNames();
    var portNames = ["WEBSERIAL"];
    setState(() {
      _portName = "WEBSERIAL";
    });
  ...
  String buildLog(LogDirection dir, String msg) {
    ...
    switch (dir) {
      case LogDirection.input:
        dateStr =
            "${now.hour.toString()}:${now.minute.toString().padLeft(2, '0')}:${now.second.toString().padLeft(2, '0')}.${now.millisecond.toString().padLeft(3, '0')}";
        dirStr = "<-";
        break;
...
/*
  void onDataRecived(ReadCommEventArgs? args) {
    if (args != null) {
      if (args.cts != _ctsEnabled || args.dsr != _dsrEnabled) {
        ...
        _logController.text +=
            buildLog(LogDirection.input, "CTS: $_ctsEnabled DSR: $_dsrEnabled");
      ...
      if (args.dataLen > 0) {
        if (_waitingForData != null) {
          if (_waitingForData!.length <= _comm.available()) {
            ...
            if (isEqual(_waitingForData!, dataread) == false) {
              _logController.text += buildLog(
                  LogDirection.none,
                  "Error compare! recived:" +
                      Uint8ListToString(dataread) +
                      " expected: " +
          ...
          String data = Uint8ListToString(_comm.read(args.dataLen));
  ...
  bool onMacroClick(KeyboardKey key, {bool clear = false}) {
    ...
    if (clear && _macroList[map].isEmpty == false) {
      ...
      if (_macroList[map].isEmpty) {
        ...
        _logController.text +=
            buildLog(LogDirection.none, "MACRO Programmed...");
        _logScrollController
            .jumpTo(_logScrollController.position.maxScrollExtent);
  ...
  KeyEventResult handleKeyEvent(FocusNode node, KeyEvent event) {
    if (event is KeyDownEvent) {
      return onMacroClick(event.logicalKey)
          ? KeyEventResult.handled
          : KeyEventResult.ignored;
    } else if (event is KeyRepeatEvent) {
      return onMacroClick(event.logicalKey, clear: true)
          ? KeyEventResult.handled
          : KeyEventResult.ignored;
  ...
  Future<void> onSendAction(String value) async {
    ...
    if (_macroMode) {
      ...
      if (data.length > 1) {
        ...
        _dev.writeDev(data[0]);
      }
    } else {
      _dev.writeDev(StringToUint8List(value));
    }
  ...
  void onPortOpen() async {
    if (_isCommOpen) {
      ...
      // _logController.text +=
      //     buildLog(LogDirection.none, "Close browser tab to close port...");
      // _settings.put("log_string", _logController.text);
      // _settings.flush();

      //_logController.text += buildLog(
      //     LogDirection.none, "Close browser tab to close Serial Port...");
      // _settings.put("log_string", _logController.text);
      // _settings.flush();
    } else {
      /*
      ...
      if (await _dev.openDev({
        ...
        _logController.text += buildLog(LogDirection.none,
            "Port openned $_portName $_baudRate $_byteSize $_parity $_stopBit $_flowControl...");
        _logScrollController
            .jumpTo(_logScrollController.position.maxScrollExtent);

        _readTimerCancel = false;
        _readTimer =
            Timer.periodic(const Duration(milliseconds: 5), (timer) async {
          if (_readTimerCancel) timer.cancel();
          final rawdata = await _dev.readDev();

          if (rawdata.isNotEmpty) {
            String data = Uint8ListToString(rawdata);
            setState(() {
              _logController.text += buildLog(LogDirection.input, data);
              _logScrollController
                  .jumpTo(_logScrollController.position.maxScrollExtent);
  ...
  void initState() {
    ...
    setState(() {
      ...
      _sendTextHistory =
          _settings.getStringList("send_history", "  **;** <BEL>");
      _macroList = _settings.getStringList("macro_list",
          "  **;** <BEL> **;** <ENQ> **;** <DLE> **;** ABC **;**  **;**  **;** ");
                    ...
                    Row(
                      children: [
                        ...
                        DropdownMenu(
                            ...
                            dropdownMenuEntries:
                                _portNames.map<DropdownMenuEntry<String>>(
                              (val) {
                                final splitted = val.split(" - ");
                                return DropdownMenuEntry(
                                    value: splitted[0], label: val);
                              },
                            ).toList()),
                        SizedBox(
                          width: 8,
                        ...
                        /*Text(
                          "Use to this button\nselect WebSerial ->",
                          style:
                              TextStyle(fontSize: 10, color: Color(0xFF000000)),
                        ),*/
                        /*IconButton(
                        ...
                        DropdownMenu(
                            ...
                            dropdownMenuEntries:
                                _baudRates.map<DropdownMenuEntry<String>>(
                              (val) {
                                return DropdownMenuEntry(
                                    value: val, label: val);
                              },
                        ...
                        Row(
                          children: [
                            ...
                            DropdownMenu(
                                ...
                                dropdownMenuEntries: _flowControls
                      ...
                      children: [
                        ...
                        Expanded(
                          child: DropdownMenu(
                              ...
                              initialSelection: _sendTextHistory.length > 1
                                  ? _sendTextHistory[1]
                                  : _sendTextHistory[0],
                              dropdownMenuEntries: _sendTextHistory
                                  .map<DropdownMenuEntry<String>>(
                                (val) {
                                  return DropdownMenuEntry(
                                      value: val, label: val);
                        ...
                        FilledButton(
                          ...
                          child: Row(
                            children: [
                              Icon(Icons.send),
                              Text(_waitingForData == null ? "SEND" : "CANCEL")
                            ],
                          ),
                        ),
                        SizedBox(
                      ...
                      children: [
                        SizedBox(
                            ...
                            child: ElevatedButton(
                                onPressed: _isCommOpen
                                    ? () => onMacroClick(LogicalKeyboardKey.f1)
                                    : null,
                                child: Column(
                                  children: [
                                    Text("F1"),
                                    Text(
                                      getStringShortut(_macroList[0]),
                                      style: TextStyle(fontSize: 10),
                                    ),
                                  ],
                                ))),
                        ...
                        SizedBox(
                            ...
                            child: ElevatedButton(
                                onPressed: _isCommOpen
                                    ? () => onMacroClick(LogicalKeyboardKey.f2)
                                    : null,
                                child: Column(
                                  children: [
                                    Text("F2"),
                                    Text(
                                      getStringShortut(_macroList[1]),
                                      style: TextStyle(fontSize: 10),
                                    ),
                                  ],
                                ))),
                        ...
                        SizedBox(
                            ...
                            child: ElevatedButton(
                                onPressed: _isCommOpen
                                    ? () => onMacroClick(LogicalKeyboardKey.f3)
                                    : null,
                                child: Column(
                                  children: [
                                    Text("F3"),
                                    Text(
                                      getStringShortut(_macroList[2]),
                                      style: TextStyle(fontSize: 10),
                                    ),
                                  ],
                                ))),
                        ...
                        SizedBox(
                            ...
                            child: ElevatedButton(
                                onPressed: _isCommOpen
                                    ? () => onMacroClick(LogicalKeyboardKey.f4)
                                    : null,
                                child: Column(
                                  children: [
                                    Text("F4"),
                                    Text(
                                      getStringShortut(_macroList[3]),
                                      style: TextStyle(fontSize: 10),
                                    ),
                                  ],
                                ))),
                        ...
                        SizedBox(
                            ...
                            child: ElevatedButton(
                                onPressed: _isCommOpen
                                    ? () => onMacroClick(LogicalKeyboardKey.f5)
                                    : null,
                                child: Column(
                                  children: [
                                    Text("F5"),
                                    Text(
                                      getStringShortut(_macroList[4]),
                                      style: TextStyle(fontSize: 10),
                                    ),
                                  ],
                                ))),
                        ...
                        SizedBox(
                            ...
                            child: ElevatedButton(
                                onPressed: _isCommOpen
                                    ? () => onMacroClick(LogicalKeyboardKey.f6)
                                    : null,
                                child: Column(
                                  children: [
                                    Text("F6"),
                                    Text(
                                      getStringShortut(_macroList[5]),
                                      style: TextStyle(fontSize: 10),
                                    ),
                                  ],
                                ))),
                        ...
                        SizedBox(
                            ...
                            child: ElevatedButton(
                                onPressed: _isCommOpen
                                    ? () => onMacroClick(LogicalKeyboardKey.f7)
                                    : null,
                                child: Column(
                                  children: [
                                    Text("F7"),
                                    Text(
                                      getStringShortut(_macroList[6]),
                                      style: TextStyle(fontSize: 10),
                                    ),
                                  ],
                                ))),
                        ...
                        SizedBox(
                            ...
                            child: ElevatedButton(
                                onPressed: _isCommOpen
                                    ? () => onMacroClick(LogicalKeyboardKey.f8)
                                    : null,
                                child: Column(
                                  children: [
                                    Text("F8"),
                                    Text(
                                      getStringShortut(_macroList[7]),
                                      style: TextStyle(fontSize: 10),
                                    ),
                                  ],
                                ))),
